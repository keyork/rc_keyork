...
HTTP 层直接使用 Go 标准库 `net/http` + `ServeMux`，不引入第三方框架。当前仅有 5 个端点、路由简单，框架带来的收益不足以抵消引入唯一外部依赖的成本。当端点数量增多、需要统一中间件（日志、限流、鉴权）时，迁移到 Gin 是自然的演进方向。
...
| 五：HTTP 框架 | 端点持续增多，中间件需要统一管理 | 迁移到 Gin，使用路由分组、中间件链和请求绑定能力 |
